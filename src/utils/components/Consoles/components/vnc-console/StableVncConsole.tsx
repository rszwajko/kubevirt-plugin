...
export const StableVncConsole: FC<StableVncConsoleProps> = ({
  ...
  const setVncState = useCallback(
    (producer: (state: ConsoleComponentState) => ConsoleComponentState) =>
      setState((oldState) => (oldState.type === VNC_CONSOLE_TYPE ? producer(oldState) : oldState)),
    [setState],
  );
  ...
  const postDisconnectCleanup = useCallback(() => {
    ...
    setVncState((state) => ({
      ...state,
      actions: { connect: state.actions.connect, disconnect: state.actions.disconnect },
      state: disconnected,
    }));
  }, [setVncState]);
  ...
  const connect = useCallback(() => {
    setVncState((state) => ({ ...state, state: connecting }));
    ...
    rfbInst.addEventListener(
      ...
      () =>
        ...
        setVncState((state) => ({ ...state, state: connected })),
    );
  ...
  useEffect(() => {
    if (!rfbRef.current) {
      initLogging('debug');
      connect();
      setVncState((state) => ({ ...state, actions: { connect, disconnect } }));
    }
    return () => {
      if (rfbRef.current) {
        disconnect();
